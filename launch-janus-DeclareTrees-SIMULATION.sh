...
# experiment folders
EXPERIMENT_NAME="experiments/SIMULATION/"${LOG_NAME}"/clusters_"${CLUSTERING_POLICY}"-treeSplit_"${SPLIT_POLICY}
INPUT_FOLDER="experiments/SIMULATION/00-INPUT-LOGS-MODELS"
...
ORIGINAL_MODEL_1=${INPUT_FOLDER}/${LOG_NAME}"-original-model-1.json"
ORIGINAL_MODEL_2=${INPUT_FOLDER}/${LOG_NAME}"-original-model-2.json"
MIN_STRLEN=10
MAX_STRLEN=50
TESTBED_SIZE=100
MEMORY_MAX="10048m"
ORIGINAL_GENERATED_LOG_1=${INPUT_FOLDER}/${LOG_NAME}"-original-log-1.xes"
ORIGINAL_GENERATED_LOG_2=${INPUT_FOLDER}/${LOG_NAME}"-original-log-2.xes"
...
# Input log
#LOG_NAME="BPIC13"
INPUT_LOG=$INPUT_FOLDER"/"$LOG_NAME"-merged-log.xes"
LOG_ENCODING="xes"
...
#
# Simulate logs
echo "################################ LOGs SIMULATION"
# LOG1
java -Xmx$MEMORY_MAX -cp Janus.jar $LOG_MAINCLASS \
  --input-model-file ${ORIGINAL_MODEL_1} \
  --input-model-encoding $MODEL_ENCODING \
  --size $TESTBED_SIZE \
  --minlen $MIN_STRLEN \
  --maxlen $MAX_STRLEN \
  --out-log-encoding $LOG_ENCODING \
  --out-log-file $ORIGINAL_GENERATED_LOG_1 \
  -d none
# LOG2
java -Xmx$MEMORY_MAX -cp Janus.jar $LOG_MAINCLASS \
  --input-model-file ${ORIGINAL_MODEL_2} \
  --input-model-encoding $MODEL_ENCODING \
  --size $TESTBED_SIZE \
  --minlen $MIN_STRLEN \
  --maxlen $MAX_STRLEN \
  --out-log-encoding $LOG_ENCODING \
  --out-log-file $ORIGINAL_GENERATED_LOG_2 \
  -d none
#merge
python3 ClusterMind/utils/merge_logs.py $INPUT_LOG $ORIGINAL_GENERATED_LOG_1 $ORIGINAL_GENERATED_LOG_2
...
if test -f "${MODEL}"; then
  ...
  java -cp Janus.jar $JANUS_DISCOVERY_MAINCLASS -iLF $INPUT_LOG -iLE $LOG_ENCODING -c $CONFIDENCE -s $SUPPORT -i 0 -oJSON ${MODEL} -oCSV ${MODEL}.csv
